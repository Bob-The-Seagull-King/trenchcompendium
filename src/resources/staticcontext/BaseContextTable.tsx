...
export const BaseContextCallTable : CallEventTable = {
    ...
    scar_heal: {
        event_priotity: 0,    
        async onSelectPropertyValue(this: EventRunner, eventSource : any, trackVal : WarbandProperty, context_func : ContextEventEntry, context_static : ContextObject, context_main : DynamicContextObject | null, warband : UserWarband | null) {
            const MemberModule = await import("../../classes/saveitems/Warband/Purchases/WarbandMember")
            
            for (let i = 0; i < trackVal.SelfDynamicProperty.Selections.length; i++) {
                const Sel = trackVal.SelfDynamicProperty.Selections[i]

                if (Sel.SelectedChoice != null) {
                    if (Sel.SelectedChoice.value instanceof MemberModule.WarbandMember) {
                        if (context_func["count"]) {
                            Sel.SelectedChoice.value.ScarReserve += context_func["count"]
                        }
                        if (context_func["all"]) {
                            Sel.SelectedChoice.value.ScarReserve = Sel.SelectedChoice.value.GetInjuriesList().length;
                        }
                    }
                }
            }
